...
当前实现状态（2026-08-26）：内置深色与浅色主题已使用同一组语义令牌驱动 Launcher、底部搜索窗、设置窗和条目添加/编辑窗。设置窗切换后即时刷新已打开的 Launcher、搜索和设置窗口，后续打开的条目编辑器使用当前主题；选择保存到 `config.json`。外部 `Themes/<name>/theme.json` 目录发现、解析、图片资源和高对比度覆盖仍未实现，以下 V1 目录格式不能描述为当前已支持。
