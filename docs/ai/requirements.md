...
| ID | 需求 | 规范 | 最低证据 | 当前状态 |
| --- | --- | --- | --- | --- |
| PROD-GRID-001 | Grid 优先、Tab 分类并支持跨 Tab 搜索 | `product-scope.md`、`launcher-ui.md` | UI 集成 + 键盘运行验证 | 部分验证（2026-08-26：`items.json` Tab/Grid 绑定、鼠标/键盘分类切换与启动、空状态、底部附属搜索窗、DIP 命中、可见焦点，以及直接输入/Ctrl+F、Unicode 跨 Tab 搜索、稳定排序、退格/粘贴、方向/Home/End、Enter 和两阶段 Esc 已实现；窗口级集成测试与 Debug 便携运行验证通过，滚动交互未实现） |
...
| UIA-001 | 键盘和 UI Automation 可操作 | `accessibility.md` | Narrator + Accessibility Insights | 部分验证（2026-08-26：Grid/Tab/搜索键盘路径、Enter、两阶段 Esc 与可见焦点已实现并完成真实窗口消息验证；UIA Provider、Narrator、Accessibility Insights 和高对比度未验证） |
| QUALITY-LOG-001 | 诊断日志受容量、数量、时间和隐私边界约束，并记录关键生命周期及未处理异常 | `quality.md` | 单元测试 + Windows 启动/退出日志验证 | 已验证（2026-08-26：10 个 CTest 全通过；覆盖净化、容量、数量、过期和异常记录，Debug 便携运行日志覆盖数据加载、窗口/服务创建与正常退出；配置关闭开关待后续实现） |
