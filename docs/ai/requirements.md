...
| UI-EFFECT-001 | 无边框窗口支持系统材质、表面 alpha、整体透明度和一致主题，并可安全降级 | `launcher-ui.md`、`themes.md` | 参数单元测试 + 各材质/主题 Windows 运行截图 + 高对比度验证 | 部分验证（2026-08-26：参数测试、四种材质窗口创建与 DWM 属性读取、内置深浅主题语义令牌、单窗口设置切换和配置兼容测试通过；外部主题、完整主题截图矩阵和高对比度未验证） |
...
| PLAT-TRAY-001 | 托盘显示/隐藏、设置、退出与 Explorer 恢复 | `windows-integration.md` | 生命周期集成测试 + Explorer 运行验证 | 部分验证（2026-08-26：真实图标矩形、选择回调 Toggle、设置入口、可复用主题设置窗和生命周期测试通过；完整设置项、菜单人工操作与 Explorer 重启验证未完成） |
