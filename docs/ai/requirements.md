...
| ID | 需求 | 规范 | 最低证据 | 当前状态 |
| --- | --- | --- | --- | --- |
| UI-POPUP-001 | 面板弹出时置顶，外部点击和前台切换时自动收起，自身设置、搜索、菜单和模态操作除外；保持打开独立控制自动收起 | `launcher-ui.md`、`activation-edge.md` | 跨进程未取得前台焦点、外部点击不丢失、边角重复唤起及内部交互运行验证 | 已验证（2026-09-22：窗口及边缘测试 67/67 用例通过；`hlaunch_popup_interaction_tests` 通过 30 次真实左上角采样/外部点击/重入循环，以及所属窗口、模态保护、原生菜单和前台切换验证，焦点场景与范围见 `quality.md`）；此条取代下方历史置顶/取消置顶行为 |
...
| PLAT-AUTOSTART-001 | 当前用户主动启用或禁用登录启动 | `product-scope.md`、`windows-integration.md` | Task Scheduler 集成、旧 Run 迁移、实际登录启动 | 部分验证（2026-09-12：普通权限的当前用户登录任务、无额外登录延迟、正常进程优先级、无运行时限、电池运行、特殊字符路径、便携参数、任务禁用、失败保留旧值与无关任务保护由自动测试覆盖；本机手动触发通过，实际重新登录待验证） |
...
| RELEASE-001 | x64、x86、ARM64 各自 `/MT` 单 EXE、无第三方运行时 DLL | `engineering.md`、`quality.md`、`releasing.md` | 三架构干净 Release 构建 + PE/内嵌 Manifest/版本/依赖检查 | 已验证（2026-09-08：v0.1.20 的三架构 GitHub Actions 构建成功，x64/x86 各 11 项非交互桌面 CTest 通过；Release 资产回读验证 SHA-256、PE/Manifest 架构、0.1.20 版本资源和无动态 CRT 导入。ARM64 是交叉编译与静态核验，真机未验证；运行矩阵仍按各需求单独记录） |
| RELEASE-TAG-001 | 仅精确版本标签触发编译，版本/日志一致，三架构齐全后创建 Release | `releasing.md`、`ADR-0007-tagged-multiarch-release.md` | 版本工具回归测试 + 工作流语法检查 + GitHub 标签发布/资产回读 | 已验证（2026-09-08：5 项 Python 测试与 actionlint 1.7.12 通过；普通配置提交未产生构建，v0.1.20 标签触发 Actions 34184056371 并完成三架构发布；远端标签指向、四份发布资产及 SHA-256、正文与 CHANGELOG.md 一致性已回读核对） |
...
2026-08-29 `PLAT-AUTOSTART-001` 入口迁移补充：v0.1.18 将开机自启从设置常规页迁移到托盘右键菜单。菜单项按实时状态显示勾选，点击后立即切换；读取失败时禁用，写入失败时显示错误。设置窗口不再创建开机自启控件，托盘测试锁定菜单顺序、标签、勾选和不可用状态。

2026-09-12 `PLAT-AUTOSTART-001` 状态源补充：自启状态真相从 HKCU Run 值改为当前用户登录计划任务（`decisions/ADR-0008-logon-task-startup.md`）。托盘菜单勾选改为按任务来源、启用状态、目标用户、权限级别、登录触发器、启动参数和 EXE 路径综合判定，查询与修改仍在后台执行；旧 Run 值只在任务创建成功后迁移。
...
2026-09-15 `PROD-DROP-001` / `PLAT-SHELL-001` 导入独立性补充：拖入普通快捷方式保存实际目标、逻辑参数、工作目录、图标及管理员标志；特殊链接保留到数据根 `shortcuts/`，没有条目覆盖时由 Shell 完整打开。验收要求删除来源 `.lnk` 后仍可启动。`tests/drop_item_resolver_tests.cpp` 的 11 个用例、185 个断言及 `tests/shell_launcher_tests.cpp` 通过，覆盖参数、工作目录、特殊链接、包身份属性和复制失败；x64 Release 构建通过。整体回归仍有既有窗口置顶用例失败，详见 `quality.md`。
