...
| PROD-ITEM-001 | 添加、编辑和跨 Tab 移动条目并安全持久化 | `launcher-ui.md`、`data-and-config.md` | 原生窗口集成 + Core 变更测试 + 后台存储测试 | 已验证（2026-08-26：点击添加与右键编辑已接入；Debug 窗口测试覆盖 `Insert`/`F2`、全部编辑字段、逻辑参数拆分、跨 Tab 移动、稳定 UUID 和搜索索引刷新，Core/存储测试覆盖使用统计保留、首次默认分类、后台快照合并和退出刷新） |
...
| UIA-001 | 键盘和 UI Automation 可操作 | `accessibility.md` | Narrator + Accessibility Insights | 部分验证（2026-08-26：Grid/Tab/搜索键盘路径、方向键跨页、PageUp/PageDown、Enter、两阶段 Esc 与可见焦点已实现并完成真实窗口消息验证；UIA Provider、Narrator、Accessibility Insights 和高对比度未验证） |
| QUALITY-LOG-001 | 诊断日志受容量、数量、时间和隐私边界约束，并记录关键生命周期及未处理异常 | `quality.md` | 单元测试 + Windows 启动/退出日志验证 | 已验证（2026-08-26：11 个 CTest 全通过；覆盖净化、容量、数量、过期和异常记录，Debug 便携运行日志覆盖数据加载、窗口/服务创建与正常退出；配置关闭开关待后续实现） |
