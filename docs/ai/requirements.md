...
| UI-EFFECT-001 | 无边框窗口支持系统材质、表面 alpha、整体透明度和一致主题，并可安全降级 | `launcher-ui.md`、`themes.md` | 参数单元测试 + 各材质/主题 Windows 运行截图 + 高对比度验证 | 部分验证（2026-08-27：默认 95% 整体不透明度及边界参数测试、四种材质窗口创建与 DWM 属性读取、非 Solid 客户区透明清除、Acrylic 合成截图、Launcher/搜索内置深浅主题语义令牌、原生设置即时切换主题/材质/透明度、schema v1 持久化与旧配置兼容测试通过；全部现有窗口统一读取系统消息字体并响应主题/设置/颜色/字体/DWM 广播，高对比度自动使用系统色、Solid 与 100% 不透明度；设置/注册/属性按设计跟随 Windows；Mica/Tabbed/Solid 完整主题截图、外部主题和高对比度人工矩阵未验证） |
...
| ACT-EDGE-001 | 边缘默认关闭，启用后 Show only | `activation-edge.md` | 状态机单元 + 多显示器运行验证 | 部分验证（2026-08-27：热区/状态机/多屏几何单元测试、线程池左外边缘 Show-only 与重武装 Debug 运行验证；设置页已支持八方向热区、多显示器模式、边缘/角落尺寸、停留/采样/冷却时间、全屏抑制、运行时应用、输入校验和失败回滚；完整多屏、全屏应用与性能矩阵未完成） |
...
| PLAT-TRAY-001 | 托盘显示/隐藏、设置、退出与 Explorer 恢复 | `windows-integration.md` | 生命周期集成测试 + Explorer 运行验证 | 部分验证（2026-08-27：真实图标矩形、选择回调 Toggle、设置入口、可复用原生设置窗、完整外观/激活/开机启动配置、标准对话框键盘路由和生命周期测试通过；菜单人工操作与 Explorer 重启验证未完成） |
...
| PLAT-SHELL-001 | 目标、逻辑参数与工作目录分离并通过 Shell 启动 | `windows-integration.md`、`data-and-config.md` | 参数编解码集成测试 + Windows 运行验证 | 部分验证（2026-08-27：引号边界、无效 UTF-8、真实 `ShellExecuteExW` 参数回读、两 Tab 点击启动和稳定 `Hunlongyu.HLaunch` AppUserModelID 查询测试通过；文件/文件夹/URL、UAC 取消和失效目标矩阵未完成） |
| UIA-001 | 键盘和 UI Automation 可操作 | `accessibility.md` | Narrator + Accessibility Insights | 部分验证（2026-08-27：Grid/Tab/搜索键盘路径、默认无选中、导航键激活焦点、PageUp/PageDown、受选中状态约束的 Enter/`F2`/`Delete`、`Insert`、两阶段 Esc、可见焦点、系统字体和高对比度语义色已实现并完成窗口消息/字体自动测试；UIA Provider、Narrator、Accessibility Insights 和高对比度人工验证未完成） |
| QUALITY-LOG-001 | 诊断日志受容量、数量、时间和隐私边界约束，并记录关键生命周期及未处理异常 | `quality.md` | 单元测试 + Windows 启动/退出日志验证 | 已验证（2026-08-27：15 个 CTest 覆盖相关回归；日志测试覆盖净化、容量、数量、过期和异常记录，Debug 便携运行日志覆盖数据加载、窗口/服务创建与正常退出；配置关闭开关待后续实现） |
