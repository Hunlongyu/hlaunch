...
| ID | 需求 | 规范 | 最低证据 | 当前状态 |
| --- | --- | --- | --- | --- |
| PROD-GRID-001 | Grid 优先、Tab 分类并支持跨 Tab 搜索 | `product-scope.md`、`launcher-ui.md` | UI 集成 + 键盘运行验证 | 已验证（2026-08-26：`items.json` Tab/Grid 绑定、394 × 605 DIP 的 5 × 8 固定槽位、区域菜单、基本分类新增/重命名/删除、鼠标/键盘分类切换与启动、同 Tab 拖拽重排、跨 Tab 末尾移动、目标高亮、底部附属搜索窗、区域滚轮、DIP 命中、可见焦点、位置指示，以及直接输入/Ctrl+F、Unicode 跨 Tab 搜索、稳定排序、退格/粘贴、方向/Home/End、PageUp/PageDown、Enter 和两阶段 Esc 已实现；Core、窗口级拖拽与区域菜单测试以及 Debug 运行验证通过） |
...
| UI-EFFECT-001 | 无边框窗口支持系统材质、表面 alpha、整体透明度和一致主题，并可安全降级 | `launcher-ui.md`、`themes.md` | 参数单元测试 + 各材质/主题 Windows 运行截图 + 高对比度验证 | 部分验证（2026-08-26：参数测试、四种材质窗口创建与 DWM 属性读取、Launcher/搜索内置深浅主题语义令牌、原生设置切换和配置兼容测试通过；设置/注册/属性按设计跟随 Windows 而非自绘主题；外部主题、完整主题截图矩阵和高对比度未验证） |
...
| PLAT-TRAY-001 | 托盘显示/隐藏、设置、退出与 Explorer 恢复 | `windows-integration.md` | 生命周期集成测试 + Explorer 运行验证 | 部分验证（2026-08-26：真实图标矩形、选择回调 Toggle、设置入口、可复用原生设置窗、标准对话框键盘路由和生命周期测试通过；完整设置项、菜单人工操作与 Explorer 重启验证未完成） |
