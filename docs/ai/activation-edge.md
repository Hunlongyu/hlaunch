...
- `ACT-EDGE-001.state-and-layout` 覆盖默认关闭、八方向、排他边界、负坐标、目标 DPI、16 DIP 完整角落、L 形接缝、状态机、抑制、时钟回退、冷却和工作区定位。
