...
当前实现状态（2026-08-27）：功能保持默认关闭。原生设置窗可启用/禁用边缘唤起并切换全屏抑制，保存后立即启动或停止线程池采样，不需要重启；启动或保存失败时恢复原服务状态。启用后使用 `CreateThreadpoolTimer` 采样，缓存多显示器几何与 DPI，支持八方向、角落优先、Every monitor 和 L 形可见外轮廓判断；状态机覆盖停留、切区重计时、离开、冷却、抑制和时钟回退。后台回调只投递激活上下文，UI 线程按触发边或角定位并只执行 Show。边缘位置、停留时间与多显示器模式的高级设置控件尚未实现；完整多屏、游戏、远程桌面与性能矩阵仍待验证。
