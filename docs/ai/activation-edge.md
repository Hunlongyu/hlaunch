...
当前实现状态（2026-08-28）：功能保持默认关闭。原生设置窗可配置八方向热区、多显示器模式、边缘宽度、角落大小、停留/采样/冷却时间、全屏抑制和前台进程黑/白名单；关闭总开关时高级字段统一禁用，越界输入就地提示并聚焦对应字段。保存后立即重启或停止线程池采样，不需要重启进程；启动或保存失败时恢复原服务状态。启用后使用 `CreateThreadpoolTimer` 采样，缓存多显示器几何与 DPI，支持角落优先、Every monitor 和 L 形可见外轮廓判断；状态机覆盖停留、切区重计时、离开、冷却、抑制和时钟回退。后台回调只投递激活上下文，UI 线程按触发边或角定位并只执行 Show。完整多屏、游戏、远程桌面与性能矩阵仍待验证。
...
允许范围：边缘 2–16 DIP；角落 8–64 DIP；停留 100–1000 ms；采样 30–50 ms；冷却 0–5000 ms。设置 UI 和配置解析必须执行相同的上下限校验，不能直接信任输入或 JSON。
...
进程名单按前台可执行文件名匹配，忽略大小写，不接受路径或通配符，每个名单最多 64 项。黑名单为空时不额外限制；白名单非空时只允许名单中的前台进程；同名项同时出现在两边时白名单优先。桌面、Shell 和 HLaunch 自身不受名单限制。只在鼠标命中热区且前台 HWND 变化时刷新并缓存进程信息，不能每次采样都重复查询。读取进程名失败时，纯黑名单模式保持可用，白名单模式按未获允许处理。
