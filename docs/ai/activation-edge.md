...
当前实现状态（2026-08-26）：功能保持默认关闭。启用后使用 `CreateThreadpoolTimer` 采样，缓存多显示器几何与 DPI，支持八方向、角落优先、Every monitor 和 L 形可见外轮廓判断；状态机覆盖停留、切区重计时、离开、冷却、抑制和时钟回退。后台回调只投递激活上下文，UI 线程按触发边或角定位并只执行 Show。全屏检测已实现 DWM 边界、不可见/最小化/cloaked/Shell 排除和失败回退；完整多屏、游戏、远程桌面与性能矩阵仍待验证。
