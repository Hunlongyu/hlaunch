...
当前已有 CMake/FetchContent 工程基线、数据/config 模型与 Glaze 编解码、标准/便携目录、原子写入与 `.bak` 回退、诊断日志与未处理异常记录、单实例应用壳、全局快捷键、默认关闭的屏幕边缘停留唤起、托盘入口，以及可运行的无边框竖向 Direct2D 界面。Tab 和 Grid 已绑定 `items.json`，支持鼠标与键盘切换、可见条目焦点及 Windows Shell 启动。搜索、滚动/分页、添加编辑、拖放、真实图标和 UI Automation Provider 尚未实现；后续仍须分别记录代码、测试、构建和运行证据。
