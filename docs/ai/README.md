...
当前已有 CMake/FetchContent 工程基线、数据/config 模型与 Glaze 编解码、标准/便携目录、原子写入与 `.bak` 回退、诊断日志与未处理异常记录、单实例应用壳、全局快捷键、默认关闭的屏幕边缘停留唤起、托盘入口，以及可运行的无边框竖向 Direct2D 界面。Tab 和 Grid 已绑定 `items.json`，支持鼠标与键盘切换、可见条目焦点、滚轮/分页及 Windows Shell 启动；底部附属搜索窗已接入跨 Tab 检索、Unicode 不区分大小写匹配、稳定排序、跨页浏览和键盘启动。原生条目编辑器已接入添加、编辑、删除、跨 Tab 移动、稳定 ID 保留、搜索索引刷新和后台串行原子保存；删除支持 `Delete`、右键菜单和默认“否”的确认。Grid 条目已支持同 Tab 拖拽重排与拖到目标 Tab 末尾，超过系统拖拽阈值后才进入排序并显示目标轮廓。OLE 拖放已接入文件、文件夹、`.exe`、`.lnk` 与 URL 的后台解析、批量顺序和完全重复提示。Windows Shell 图标已在专用后台线程提取，WIC 转换结果投递 UI 线程后创建并缓存 Direct2D 位图，失败保留首字符占位。Tab 管理和 UI Automation Provider 尚未实现；后续仍须分别记录代码、测试、构建和运行证据。
