...
当前已有 CMake/FetchContent 工程基线、数据/config 模型与 Glaze 编解码、标准/便携目录、原子写入与 `.bak` 回退、单实例应用壳、全局快捷键、默认关闭的屏幕边缘停留唤起、托盘入口，以及可运行的无边框竖向 Direct2D 界面骨架。界面中的搜索、Tab、Grid 和条目均为静态呈现，不能据此推导搜索、导航、启动、拖放或 UI Automation 已实现。后续仍须分别记录代码、测试、构建和运行证据。
