...
- `platform/windows`：Shell、OLE 拖放、注册表、托盘、DPI、显示器、进程和 IPC。
...
`ActivationContext` 至少包含：触发类型、鼠标位置、目标显示器、热区和请求行为（show/toggle）。不要为尚未实现的触发器预留会被误当成支持范围的枚举值；新增 Tray/IPC 触发时再加入或标记清楚。
