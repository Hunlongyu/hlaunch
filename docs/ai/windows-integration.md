...
当前实现状态（2026-08-26）：已实现按当前用户 SID 隔离的 Named Mutex、固定类名隐藏激活窗口，以及 `--show`、`--hide`、`--toggle` 的注册消息转发；同一完整性级别下实测第二实例退出且主实例保持唯一。有限重试、不同完整性级别 UIPI、托盘和开机启动仍未实现。UI 阶段为保证可见和可退出，主实例无参数启动时暂时直接显示；全局快捷键和托盘完成后再恢复“启动后隐藏”的产品默认值。
