...
当前实现状态（2026-08-26）：已实现按当前用户 SID 隔离的 Named Mutex、固定类名隐藏激活窗口、`--show`、`--hide`、`--toggle` 的注册消息转发，以及全局快捷键注册。快捷键可用时主实例无参数启动默认隐藏；快捷键禁用或注册失败且没有显式启动命令时暂时显示主窗，保证托盘和设置页完成前仍可退出和诊断。同一完整性级别下实测第二实例退出且主实例保持唯一。有限重试、不同完整性级别 UIPI、托盘和开机启动仍未实现。
