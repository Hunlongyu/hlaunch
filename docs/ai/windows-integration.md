...
Shell 图标转换结果使用数据根 `cache/icons` 中的私有二进制缓存。缓存格式 v4 按条目 UUID 建立命名空间，读取前校验格式版本、完整身份、像素尺寸、文件长度和内容校验和；缓存键还包含源文件时间与大小。删除条目时，UI 先从当前 `ItemsDocument` 移除并提交持久化快照，同时立即清理该 UUID 的内存图标，磁盘缓存清理由图标工作线程串行执行并在正常退出前完成；重新添加同路径或同名程序会获得新 UUID，不得命中已删除条目的缓存。EXE 图标优先通过资源提取直接读取文件，再降级到 Shell 图像工厂，避免 Windows Shell 按路径复用旧图标。改变解码、Alpha、缩放或缓存身份语义时必须提升缓存格式版本，使旧像素自动失效而不能继续显示。损坏或过期条目直接重新提取，缓存最多保留 256 项或 64 MiB，淘汰只处理 `.hlci` 文件。
...
当前实现仅在 UI 线程读取 `IDataObject` 并复制路径或 URL 字符串，不把 COM 接口跨线程传递。文件系统路径落在已有应用或快捷方式 Grid 条目时，保留该条目已配置的逻辑参数，再按拖入顺序把每个路径追加为一次性逻辑参数并立即启动；不修改或新增持久化条目。落在空槽、非程序条目、Tab 或其他区域时，后台解析器按来源顺序生成领域条目，并保留 UI 线程命中的目标 Tab 与可选 `gridSlot`；Grid 精确槽位连续导入时，已占用的连续槽位向后顺移，Tab 或其他区域则追加到对应分类末尾。浏览器 URL 拖入仅使用 Windows 标准 OLE 数据格式，不提供或要求浏览器扩展；脚本式 URL scheme（`javascript:`、`vbscript:`、`data:`）拒绝导入。OLE 数据对象集成测试覆盖文件顺序、URL、复制效果和不支持格式；2026-08-29 用户真实鼠标验证 Explorer 文件、文件夹、多文件、目标 Tab、空槽、占用槽、已有程序条目一次性参数、Unicode 路径和重复项目处理均通过，浏览器 URL 拖入此前亦已人工验证通过。
...
- 注册消息只服务于 HLaunch 自身实例，且只能承载 `show`、`hide`、`toggle` 小型命令标识，不传指针、字符串或复杂数据。窗口发现、同用户隔离及不同完整性级别下的 UIPI 行为必须运行验证。
- 找不到主窗口时不立即抢占仍被 Mutex 持有的实例；有限重试耗尽后明确提示主实例尚未就绪。消息投递失败时区分访问被拒绝、主实例无响应和其他系统错误。若 UIPI 因完整性级别不同阻止消息，提示用户以同一权限级别运行，不放宽消息过滤器。
- 不提供 Named Pipe、`WM_COPYDATA`、外部 CLI 或其他通用控制协议；不得把单实例注册消息记录成第三方可调用接口。
