...
- `shortcuts/`：拖入时无法完整转换的快捷方式副本，属于用户数据；备份或搬移数据根时一同保留，不能按缓存清理。
...
- 新拖入的普通 `.lnk` 保存解析后的目标类型、目标、逻辑参数、工作目录、图标和管理员标志；保留快捷方式显示名称。具有特殊属性、非默认窗口显示方式或非零图标索引的链接保留为 `shortcut`，指向数据根 `shortcuts/` 内的独立副本。副本按内容索引并核对完整字节，相同内容复用；单个副本上限为 1 MiB，复制失败或链接损坏时跳过导入并提示，不能退回依赖原文件。
- 此行为沿用 schema v1；历史 `shortcut` 条目仍按原目标读取，不自动猜测已删除快捷方式的目标。重新拖入现存快捷方式可使用新策略。保留副本暂不自动删除，以免破坏重复条目或备份的引用。
