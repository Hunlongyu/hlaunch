...
当前实现状态（2026-08-27）：已建立独立 Core 模型和 Glaze DTO 适配层，支持 `config.json`、`items.json` 的内存编解码、字段/范围校验、高版本只读保护、输入规模限制和失效 item 隔离；已实现标准/便携数据目录、同目录临时文件、刷盘关闭、原子替换、单一 `.bak` 和损坏主文件回退。条目修改使用后台串行工作线程保存，尚未开始写入的多个快照合并为最新值，退出前等待最终快照提交；失败会写诊断日志并通知 UI。设置窗口可同步原子保存外观、快捷键和全部 `activation.screenEdge` 字段；激活设置遵循“运行时服务成功后保存，任一步失败则恢复旧服务”的事务顺序。开机启动以当前用户 Run 注册表值为状态真相，不写入 `config.json`。旧版缺失 `appearance` 的 schema v1 配置按 Dark/Acrylic/95% 读取，只有 `appearance.theme` 的旧配置也使用 Acrylic/95% 补全。未知字段尚未逐字段生成诊断，通用配置异步保存、恢复确认 UI 和跨 schema 迁移仍未实现，因此 `DATA-CONFIG-001` 仍是部分验证。
